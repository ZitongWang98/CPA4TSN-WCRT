...
class ATSSchedulerAblation(ATSSchedulerOpt):
    """Parameterized ATS scheduler for ablation study.

    Toggles three optimizations independently:
        use_token_tracking: per-frame token state (vs bucket-empty)
        use_group_et:       group eligible time
        use_token_limit:    min(eta_plus, n_token) interference counting
    """

    def __init__(self, use_token_tracking=True, use_group_et=True,
                 use_token_limit=True):
        super().__init__()
        self._use_token_tracking = use_token_tracking
        self._use_group_et = use_group_et
        self._use_token_limit = use_token_limit

    def _eT_block(self, task, q, resource):
        if not self._use_token_tracking:
            return _compute_eT_block_naive(task, q, resource)
        if not self._use_group_et:
            return _compute_eligible_time_block(task, q, resource)
        return _compute_eT_block(task, q, resource)

    def _count(self, ti, w, resource):
        n = ti.in_event_model.eta_plus_closed(w)
        if self._use_token_limit and _is_ats(ti, resource):
            n = min(n, _n_token(w, ti, resource))
        return n

    def _w_ats(self, task, q, a, resource, details=None, **kwargs):
        eT_block = self._eT_block(task, q, resource)
        eT_abs = a + eT_block
        ...
        w_tilde = lpb + spb_fixed
        for _ in range(1000):
            spb_diff = sum(self._count(ti, w_tilde, resource) * ti.wcet
                           for ti in diff_port_flows)
            hpb = 0.0
            for ti in _get_interferers(task):
                if ti.scheduling_parameter <= task.scheduling_parameter:
                    continue
                hpb += self._count(ti, w_tilde, resource) * ti.wcet
            w_new = lpb + spb_fixed + spb_diff + hpb
            if w_tilde == w_new:
                break
            w_tilde = w_new

        return eT_abs + w_tilde
    ...
    def _w_nats(self, task, q, a, resource, details=None, **kwargs):
        ...
        w = (q - 1) * task.wcet + lpb
        for _ in range(1000):
            spb = sum(ti.wcet * ti.in_event_model.eta_plus_closed(w)
                      for ti in _get_interferers(task)
                      if ti.scheduling_parameter == task.scheduling_parameter)
            hpb = 0.0
            for ti in _get_interferers(task):
                if ti.scheduling_parameter <= task.scheduling_parameter:
                    continue
                hpb += self._count(ti, w, resource) * ti.wcet
            w_new = (q - 1) * task.wcet + lpb + spb + hpb
            if w == w_new:
                break
            w = w_new
